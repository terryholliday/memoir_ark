...
import { getLedgerClient, ProvenanceTimeline } from '../services/ledger-client';
import { getCoreClient, RegisteredAsset } from '../services/core-client';
 ...
 */
router.get('/assets/:paid', async (req: Request, res: Response) => {
...
router.get('/owner/:ownerId', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const assetsWithProvenance = await Promise.all(
 ...
 */
router.get('/source/:sourceApp', async (req: Request, res: Response) => {
 ...
 */
router.get('/anchor/:anchorId', async (req: Request, res: Response) => {
 ...
 */
function summarizeEvent(source: string, eventType: string, payload: Record<string, unknown>): string {
  const typeMap: Record<string, string> = {
    ...
    // Capital events
    'CAPITAL_CLAIM_PAYOUT': `Payout: ${payload.amount_micros ? `$${(parseInt(payload.amount_micros as string) / 1000000).toFixed(2)}` : 'processed'}`,
