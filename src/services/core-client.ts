...
class CoreClient {
   ...
   */
  async getAssetsByOwner(ownerId: string): Promise<RegisteredAsset[]> {
    try {
      ...
      const data = await response.json() as any;
      return (data.assets || data || []).map((a: any) => ({
        paid: a.paid,
   ...
   */
  async getAssetsBySource(sourceApp: string, sourceId?: string): Promise<RegisteredAsset[]> {
    try {
